...
@api.before_request
def log_entry():
    info = request.method + " " + request.url + " BY " + request.remote_addr
...
#Customer resource
@api.route('/customers/<string:user_name>/<string:str_2>',methods=['GET'])
def customer_check(user_name,str_2):
    password = (base64.b64decode(str_2.encode('ascii'))).decode('utf-8')
    customer = Khach_hang.query.filter(Khach_hang.ten_dang_nhap==user_name).first()
    if customer and check_password_hash(customer.mat_khau_hash,password):
